...
- **Logical vs filing repo** — a task's **logical repo** (`repo_id`) owns its
  ...
  issues in a dedicated issues-repo (`rl workspace set-filing-repo`) or override
  per task (`rl task create --filing-repo`). The filing repo is resolved and
  recorded at promote; only `repo_id` is exposed on task JSON. See
  [`0002-task-repo-axes.md`](docs/rfcs/0002-task-repo-axes.md).

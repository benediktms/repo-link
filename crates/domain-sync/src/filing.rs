...
/// 2. the **workspace default** (`workspaces.filing_repo_id`), else
/// 3. the task's **logical `repo_id`** if set, else
/// 4. **`None`** — no filing repo, i.e. a project-board draft.
///
/// This is `explicit_override.or(workspace_default).or(logical_repo)`, but it
...
///
/// - **Orphan + workspace default** (`logical_repo == None` but
...
///   are absent. A `None` logical repo is a *failing* step-3 condition
...
pub fn resolve_filing_repo(
    explicit_override: Option<RepoId>,
    workspace_default: Option<RepoId>,
    logical_repo: Option<RepoId>,
) -> Option<RepoId> {
    explicit_override.or(workspace_default).or(logical_repo)
}
